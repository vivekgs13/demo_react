...
const Products = () => {
  ...
  const loadMoreProducts = () => {
    ...
    setTimeout(() => {
      setDisplayedProducts((prev) => Math.min(prev + 8, allProductsList.length))
      setIsLoading(false)
    }, 500)
  }
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !isLoading && displayedProducts < allProductsList.length) {
          loadMoreProducts()
        }
      },
      { threshold: 0.1 }
  ...
  }, [isLoading, displayedProducts, allProductsList.length])

  const productsList = allProductsList.slice(0, displayedProducts)

  return (
    <div className="content-section">
      <h1>Products</h1>
      <div className="products-info">
        <p>Showing {productsList.length} of {allProductsList.length} products</p>
      </div>
      ...
      {displayedProducts < allProductsList.length && (
        <div ref={observerTarget} className="scroll-trigger"></div>
      )}

      {displayedProducts >= allProductsList.length && allProductsList.length > 0 && (
